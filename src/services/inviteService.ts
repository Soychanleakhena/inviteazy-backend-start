...
export class InviteeService implements IInviteeService {
  ...
  async create(invitee: IInviteeWithoutId): Promise<IInvitee> {
    const existingInvitees = await this.inviteeRepository.findByUserId(
      invitee.user_id
    );
  ...
  async findInviteeByEventId(event_id: string): Promise<IInvitee[]> {
    return await this.inviteeRepository.findInviteeByEventId(event_id);
  }

  async countInviteeStatusByEventId(
    event_id: string
